...
so that newspro.cgi can write to files in its directory. Or, more specifically:
change the permissions on the newspro directory so that IUSR_SERVERNAME has CHANGE
access to it and the directory newspro is running on has execute access in IIS.
...
Current build: 3.7.5 build 32. Released: February 12, 2001.
...
- Perl 5.001 or higher. 99% of web servers which support CGI scripts have this.
	On Windows, you need Perl 5.005 or higher.
- Server Side Includes (SSI). 99% of web servers which support Perl have this.

	You may have to enable it, though only if it doesn't work at first; 
	if you use an Apache server (60% of Web servers are Apache), 
	create a file called .htaccess in your main directory and add the
	lines below (if it already exists, add these lines to the bottom):
